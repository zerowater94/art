define([ 'basicInfo', 'text!app/base/org/orgList.html' ], function($a, _tmpl) {
	...
	var compApp = $a.ctl({
			...
			_self.gridComp = $a.t.grid.render(_self.areaCompList,
					{
						...
						colModel : [ {
							dataIndx : "compCode",
							title : $a.getMsg("lbl.compCode"),
							width : 100,
							editable : false
						}, {
							dataIndx : "compName",
							title : $a.getMsg("lbl.compName"),
							width : 200,
							editable : false
						} ],
						...
						rightActions : {
							...
							deleteItem : {
								onClick : function(selectedItem) {
									_self.selectedCompData = $a.u
											.null2Obj(_self.gridComp
													.getSelection().rowData);
									_self.deleteCompany();
								}
							},
						},
		...
		saveCompany : function() {
			...
			var formData = $.extend(true, {}, _self.selectedCompData,
					_self.compEditor.getValues());
	...
	var deptApp = $a.ctl({
		...
		setupEditorDept : function(param) {
			...
			_self.deptEditor = $a.t.mainEditor.render({
				...
				formList : [ {
					id : "parentDeptName",
					type : "text",
					label : $a.getMsg("lbl.parentDeptName"),
					validation : {
						readOnly : true
					}
				}, {
		...
		saveDept : function() {
			...
			if (_self.selectedDeptData.isInputMode) {
				...
				formData = $.extend(true, {}, _self.selectedDeptData,
						_self.deptEditor.getValues());
			}
		...
		deleteDept : function(selectedDeptData) {
			...
			$a.print.confirm($a.getMsg("msg.confirm.detele.includeSub"),
					function() {
						$a.send({
							url : $a.getDefaultUrl() + "/base/org/dept",
							type : "delete",
	...
	var userApp = $a.ctl({
		...
		makeUserSearch : function() {
			var _self = this;
			_self.rowboxSearchUser = $a.t.search.render(this.areaUserSearch,{
			...
			this.btnAddDeptMember = this.areaUserSearch
					.find("#org_add_user");
		...
		makeUserListGrid : function() {
			var _self = this;
			_self.gridUsers = $a.t.grid.render(this.areaUserListGrid, {
				height : 150,
				colModel : [ {
					dataIndx : "userName",
		...
		deleteMember : function(deleteObj) {
			var _self = this;
		...
		searchUserListByDept : function(deptObj) {
			...
			if (!$a.u.isNull(deptObj)) {
				...
				_self.rowboxSearchUser.clearValues();
			}
			var paramData = _self.rowboxSearchUser.getValues();
		...
		searchUserDetail : function(userObj) {
			var _self = this;
			$a.send({
				url : $a.getDefaultUrl() + "/base/org/user/userId/"
						+ userObj.userId,
				type : "get",
				success : function(data) {
					_self.setupEditorUser(data);
				}
			});
		},
		setupEditorUser : function(dataObj) {
			var _self = this;
			var param = {
				title : $a.getMsg("lbl.orgUserMng"),
				formList : [ {
					...
					id : "userNickName",
					type : "text",
					label : $a.getMsg("lbl.userNickName"),
					validation : {
						readOnly : true
					},
				}, {
			...
			_self.userEditor = $a.t.mainEditor.render(param);
			_self.editorForms = _self.userEditor.getForms();
			_self.btnCheckLoginId = _self.editorForms.loginId.area
					.find("#check-loginid");
			_self.formLoginId = _self.editorForms.loginId.area
					.find("#loginId");
			_self.userEditor.showEditor({
				data : dataObj
			});
		}
	});
